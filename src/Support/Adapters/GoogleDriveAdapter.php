...
class GoogleDriveAdapter implements FilesystemAdapter
{
    ...
    protected function delete_by_id($ids)
    {
        ...
        foreach ($ids as $id) {
            if ($id !== '' && ($file = $this->getFileObject($id))) {
                if ($file->getParents()) {
                    if ($this->usePermanentDelete && $this->service->files->delete(
                        $id,
                        $this->applyDefaultParams([], 'files.delete')
                    )
                    ) {
                        $this->uncacheId($id);
                        $deleted = true;
                        ...
                        if (!$this->usePermanentDelete) {
                            ...
                            if ($this->service->files->update(
                                $id,
                                $file,
                                $this->applyDefaultParams([], 'files.update')
                            )
                            ) {
                                $this->uncacheId($id);
                                $deleted = true;
    ...
    public function read(string $location): string
    {
        ...
        if (($response = $this->service->files->get(
            /** @scrutinizer ignore-type */            $fileId,
    ...
    private function getRawVisibility($file)
    {
        ...
        if (empty($permissions)) {
            $permissions = $this->service->permissions->listPermissions(
                $file->getId(),
                $this->applyDefaultParams([], 'permissions.list')
            );
    ...
    protected function publish($path)
    {
        ...
        if (($file = $this->getFileObject($path))) {
            ...
            try {
                ...
                if ($permission = $this->service->permissions->create(
                    $file->getId(),
                    $new_permission,
                    $this->applyDefaultParams([], 'files.create')
                )
                ) {
            ...
            try {
                foreach ($permissions as $permission) {
                    if ($permission->type === $this->publishPermission['type'] && $permission->role === $this->publishPermission['role'] && !empty($file->getId())) {
                        $this->service->permissions->delete(
                            $file->getId(),
                            $permission->getId(),
                            $this->applyDefaultParams([], 'files.trash')
                        );
                    }
                }
                $file->setPermissions([]);
                        ...
                        if ($result->isDir()) {
                            ...
                            if ($recursive) {
                                $results = array_merge(
                                    $results,
                                    $this->getItems(
                                        $result->extraMetadata()['virtual_path'],
                                        true,
                                        $maxResults,
                                        $query
                                    )
    ...
    protected function uncacheId($id)
    {
        ...
        if ($basePath) {
            foreach ($this->cachedPaths as $path => $itemId) {
                if (strlen((string)$path) >= strlen($basePath) && strncmp(
                    (string)$path,
                    $basePath,
                    strlen($basePath)
                ) === 0
            ...
            if (is_array($this->cachedPaths[$tmp])) {
                ...
                foreach ($paths as $path => $obj) {
                    ...
                    foreach ($this->cachedPaths[$tmp] as $id) {
                        if ($parentId === '' || (!empty($this->cacheFileObjects[$id]->parents) && in_array(
                            $parentId,
                            $this->cacheFileObjects[$id]->parents
                        ))
                        ) {
    ...
    protected function toSingleVirtualPath(
        $displayPath,
        $makeFullVirtualPath = true,
        $can_throw = true,
        $createDirsIfNeeded = false,
        $is_dir = false
    ) {
        try {
